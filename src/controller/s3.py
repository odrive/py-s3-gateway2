...
def delete_folder(region, host, access_key, access_key_secret, bucket, object_prefix):
    ...
    while True:
        ...
        descendants_xml = util.s3.list_objects(
            region=region,
            host=host,
            access_key=access_key,
            access_key_secret=access_key_secret,
...
def iter_file(region, host, access_key, access_key_secret, bucket, object_key):
    ...
    # stream data
    return util.s3.get_data_iterator(
        region=region,
        host=host,
        access_key=access_key,
        access_key_secret=access_key_secret,
...
def rename(region, host, access_key, access_key_secret, bucket, object_key, new_name):
    ...
    source_object = util.s3.get_object(
        region=region,
        host=host,
        access_key=access_key,
        access_key_secret=access_key_secret,
...
def _upload_file(region, host, access_key, access_key_secret, bucket, object_key, size, data):
    ...
    response = util.s3.create_multipart_upload(
        region=region,
        host=host,
        access_key=access_key,
        access_key_secret=access_key_secret,
    ...
    while total_bytes_sent < file_size:
        ...
        input_stream = StreamingIterator(part_size, part)

        # upload part to multipart upload session
        part_result = util.s3.upload_part(
            region=region,
            host=host,
            access_key=access_key,
            access_key_secret=access_key_secret,
    ...
    response = util.s3.complete_multipart_upload(
        region=region,
        host=host,
        access_key=access_key,
        access_key_secret=access_key_secret,
