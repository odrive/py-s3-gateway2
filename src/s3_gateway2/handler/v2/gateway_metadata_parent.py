...
def _put_gateway_metadata(environ, params):
    ...
    # Check source.
    if s3_gateway2.util.metadata_id.object_key(params['gateway.metadata.id'])[-1] == '/':
    ...
    result = s3_gateway2.controller.s3.move(
        ...
        new_prefix=s3_gateway2.util.metadata_id.object_key(params['new.gateway.metadata.parent.id'])
    )
    if result is None:
        return {
            'code': '403',
